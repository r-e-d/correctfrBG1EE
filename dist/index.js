...
function main() {
  ...
  const tph1FD = fs.openSync('correctfrbg1ee/lib/bg1ee.tph', 'w');
  const tph2FD = fs.openSync('correctfrbg1ee/lib/sod.tph', 'w');
  const traFD = fs.openSync('correctfrbg1ee/francais/correctfr.tra', 'w');
  ...
  while (1) {
    ...
    if (refLine.text !== fixLine.text) {
      ...
      const tphFD = fixLine.nb < 34000 ? tph1FD : tph2FD;
